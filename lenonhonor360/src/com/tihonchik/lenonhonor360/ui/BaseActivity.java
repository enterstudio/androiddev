...
//		BaseCallbacks, OnDateSetListener {
...
/*		LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		View dialogView = inflater.inflate(R.layout.popup_single_error, null);
...
/*		// Grab the message and buttons from the Content Package
		String errorMessage = AppContent.getInstance()
				.getErrorString(errorCode);

		List<String> buttons;
		boolean isMissingError = errorMessage.isEmpty();

		if (isMissingError) {
			// Handle missing error message
			errorMessage = getResources().getString(
					R.string.missing_error_message);
			buttons = new ArrayList<String>();
			buttons.add("ok");
		} else {
			// Add the dynamic buttons
			buttons = AppContent.getInstance().getErrorButtons(errorCode);
		}

		// Grab the error code for debugging purposes
...
//		tv.setTypeface(FontUtils.getEffraLight());
		...
		ViewGroup scrollButtonList = (ViewGroup) dialogView
				.findViewById(R.id.ll_scrollView);
		for (String buttonType : buttons) {
			Button button;
			if (buttonType.equals(buttons.get(buttons.size() - 1))) {
				button = (Button) inflater.inflate(R.layout.inc_primary_button,
						null);
			} else {
				button = (Button) inflater.inflate(
						R.layout.inc_secondary_button, null);
			}
//			button.setTypeface(FontUtils.getEffraMedium());
			// Going to use this to test if the button is configured properly
			button.setText(""); 

			if ("ok".equalsIgnoreCase(buttonType)) {
			...
			} else if ("visitfullsite".equalsIgnoreCase(buttonType)) {
				button.setText(R.string.visit_full_website);
				button.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						// Bring up the web-site in the external browser
						Intent intent = new Intent(Intent.ACTION_VIEW);
						intent.setData(Uri
								.parse(AppDefines.LENONHONOR_WEBSITE_URI));
						startActivity(intent);
					}
				});
			} 
			// Make sure the button is configured
